...
def get_output_filename(input_path: str, output_folder: str, reserved=None) -> str:
    ...
    reserved — пути, занятые в этом запуске: два файла с одинаковым именем
    из разных папок иначе дали бы один путь. Имена прошлых запусков не
    учитываем, чтобы повторный прогон обновлял результат, а не плодил «(2)».
    """
